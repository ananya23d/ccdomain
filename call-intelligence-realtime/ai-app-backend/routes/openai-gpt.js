...
//const openaisd = require('@azure/openai')
...
//Setup OpenAI deployment params
const tenantId = config[0].tennatId;
const openaiAccountName = config[0].openAIAccountName
const accessToken = config[0].token
...
router.post('/gpt/customPrompt', async (req, res) => {
  console.log(`post call for custom prompt`)
  ...
  //https://anany-m482adid-francecentral.cognitiveservices.azure.com/openai/deployments/ccopenaidepgpt35turbo/completions?api-version=2024-10-21
  ...
  try {
    ...
    console.error('ERROR WITH AZURE OPENAI API /gpt/customPrompt:', error.message);
...
router.post('/azure/getDeployments', async (req, res) => {
  console.log(`deployments will be processed`)

  var tennatId = tenantId
  var accountName = openaiAccountName
  var url = 'https://management.azure.com/subscriptions/' + tennatId + '/resourceGroups/ccdomain/providers/Microsoft.CognitiveServices/accounts/' + accountName + '/deployments?api-version=2024-10-01'
  console.log("the url", url)
  const headers = { 'Authorization': accessToken};
  console.log("Headers\n", headers)
  try {
    const completionResponse = await axios.get(url, { headers: headers });
    console.log("response=====>", completionResponse.data.value[0].name);
    res.send(completionResponse.data.value[0].name)
  } catch (error) {
    console.error('ERROR WITH AZURE OPENAI API /azure/getDeployments:', error.message);
    res.send(error.message)
  }
});
...
router.post('/gpt/DiarizationPrompt', async (req, res) => {
  ...
  try {
    ...
    console.error('ERROR WITH AZURE OPENAI API /gpt/DiarizationPrompt:', error.message);
    res.send(error.message)
  }
});
...
router.post('/gpt/summarize', async (req, res) => {
  ...
  try {
    ...
    console.error('ERROR WITH AZURE OPENAI API /gpt/summarize:', error.message);
    res.send(error.message)
  }
});
...
async function generateSummary(text) {
  ...
  try {
    ...
    console.error('ERROR WITH AZURE OPENAI API generateSummary:', error.message);
